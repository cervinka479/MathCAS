...
def nnArch(io=[9,2], hl=[12], task='class'):    
...
def train_and_evaluate(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, task='class'):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'residual_no_shear_best_model.pth')
    ...
    df_metrics.to_csv(f'residual_no_shear_metrics.csv', index=False)
...
# Load the dataset
df = pd.read_csv('deleteme\dataset_compressible_flow_500K_training_nstep180.csv')
...
# Create DataLoaders
batch_size = 96  # Set your batch size here
...
# Create the model
model = nnArch(io=[9, 2], hl=hidden_layers, task=task).to(device)
