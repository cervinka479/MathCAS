...
def dynamically_get_activation(activation: str) -> nn.Module:
    try:
...
class NeuralNetwork(nn.Module):
    def __init__(self, config: ArchitectureConfig) -> None:
        ...
        for hidden_size in config['hidden_layers']:
            ...
            self.layers.append(dynamically_get_activation(config['activation'])())
        ...
        if final_activation := config['final_activation']:
            self.layers.append(dynamically_get_activation(final_activation)())
