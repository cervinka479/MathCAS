...
def train_and_evaluate(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, task='class'):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'shear_1024x4.pth')
    ...
    # df_metrics.to_csv(f'shear8x256_metrics.csv', index=False)
...
# Load the dataset
df = pd.read_csv('deleteme\dataset_compressible_flow_5M_A_training_nstep180.csv')
...
learning_rate = 8.727063100330734e-05
num_epochs = 500

# Create the model
model = nnArch(io=[9, 1], hl=hidden_layers, task=task).to(device)
