...
def train_and_evaluate(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs, task='class'):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'correction_best_model.pth')
    ...
    df_metrics.to_csv(f'correction_metrics.csv', index=False)
...
# Create DataLoaders
batch_size = 28  # Set your batch size here
train_loader = DataLoader(TensorDataset(X_train_tensor, y_train_tensor), batch_size=batch_size, shuffle=True)
val_loader = DataLoader(TensorDataset(X_val_tensor, y_val_tensor), batch_size=batch_size, shuffle=False)

# Set your hyperparameters here
hidden_layers = [305,305,305]
learning_rate = 1.78307912493205e-05
num_epochs = 500

# Create the model
model = nnArch(io=[12, 3], hl=hidden_layers, task=task).to(device)
