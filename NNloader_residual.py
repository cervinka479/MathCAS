...
X_test = df.iloc[:10, list(range(9)) + [14]].values
y_true = df.iloc[:10, 12:14].values

# Convert to PyTorch tensors
X_test_tensor = torch.tensor(X_test, dtype=torch.float32)

# Load the trained model
model_path = r'saved_models\residual_reg_v02.pth'  # Use raw string to handle backslashes
dropout_prob = 0.5
task = 'regression'
...
print("True vorticity:", y_true[:,0])
print("Predicted vorticity:", y_pred[:,0].flatten())
...
print("True strain-rate:", y_true[:,1])
print("Predicted strain-rate:", y_pred[:,1].flatten())
